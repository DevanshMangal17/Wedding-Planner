...
No external services are required to run the full app — see [AI provider](#5-ai-provider) below.
...
- **Engine:** SQLite via Prisma (chosen for zero-dependency local setup — no Docker/Postgres install required). The schema is standard relational SQL and would port to Postgres by changing the `datasource` provider.
...
6. Move from SQLite to Postgres for multi-instance deployment (schema is already portable — only the Prisma `datasource` provider changes).
